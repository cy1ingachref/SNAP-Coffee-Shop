...
(function () {
  var craft = document.getElementById('story');
  if (!craft) return;
  ...
  function place(id, t) {
    ...
    if (!g) return;
    var e = ease(t);
    g.setAttribute('transform', 'translate(0,' + (hiddenY[id] * (1 - e)).toFixed(1) + ')');
  ...
  function apply(p) {
    ...
    if (sCap) sCap.textContent =
      p < 0.18 ? 'Building the base…' : p < 0.36 ? 'The patty goes on…' : p < 0.52 ? 'A slice of cheese…' :
      p < 0.68 ? 'Crisp lettuce…' : p < 0.82 ? 'Fresh tomato…' : p < 0.95 ? 'Crowning the top bun…' : 'Fresh off the grill.';
    if (cCap) cCap.textContent =
      p < 0.10 ? 'Ready for a pour…' : p < 0.82 ? 'Pouring the perfect cup…' : 'Steaming and served.';
  }
  function update() {
    var rect = craft.getBoundingClientRect();
    var total = craft.offsetHeight - window.innerHeight;
    apply(total > 0 ? clamp01(-rect.top / total) : 0);
  }
  var ticking = false;
